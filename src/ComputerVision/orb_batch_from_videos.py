...
def main():
    ...
    parser.add_argument("--outdir", default="orb_batch_results")
    ...
    orb_dir = outdir / "orb"
    for d in (outdir, extracted_dir, prepared_dir, orb_dir):
    ...
    for rec_id in args.recordings:
        ...
        for i, row in enumerate(chosen, start=1):
            ...
            res = run_orb(gray_t, gray_v)

            if res["match_vis"] is not None:
                cv2.imwrite(str(orb_dir / f"{pair_name}_matches.png"), res["match_vis"])
            if res["overlay"] is not None:
                cv2.imwrite(str(orb_dir / f"{pair_name}_overlay.png"), res["overlay"])
            if res["warped_visible"] is not None:
                cv2.imwrite(str(orb_dir / f"{pair_name}_warped_visible.png"), res["warped_visible"])
    ...
    with txt_out.open("w", encoding="utf-8") as f:
        f.write("Batch ORB alignment from matched video frames\n")
        f.write("============================================\n\n")
